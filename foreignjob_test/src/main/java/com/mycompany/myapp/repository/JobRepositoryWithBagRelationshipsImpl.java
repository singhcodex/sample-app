...
public class JobRepositoryWithBagRelationshipsImpl implements JobRepositoryWithBagRelationships {
    ...
    public Optional<Job> fetchBagRelationships(Optional<Job> job) {
        return job.map(this::fetchTasks).map(this::fetchApplicants);
    }
    ...
    public List<Job> fetchBagRelationships(List<Job> jobs) {
        return Optional.of(jobs).map(this::fetchTasks).map(this::fetchApplicants).orElse(Collections.emptyList());
    }
    ...
    Job fetchApplicants(Job result) {
        return entityManager
            .createQuery("select job from Job job left join fetch job.applicants where job.id = :id", Job.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Job> fetchApplicants(List<Job> jobs) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, jobs.size()).forEach(index -> order.put(jobs.get(index).getId(), index));
        List<Job> result = entityManager
            .createQuery("select job from Job job left join fetch job.applicants where job in :jobs", Job.class)
            .setParameter("jobs", jobs)
            .getResultList();
        Collections.sort(result, (o1, o2) -> Integer.compare(order.get(o1.getId()), order.get(o2.getId())));
        return result;
