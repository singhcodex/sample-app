...
type RestOf<T extends IJob | NewJob> = Omit<T, 'expiryDate'> & {
...
export class JobService {
  ...
  protected convertDateFromClient<T extends IJob | NewJob | PartialUpdateJob>(job: T): RestOf<T> {
    return {
      ...job,
      expiryDate: job.expiryDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restJob: RestJob): IJob {
    return {
      ...restJob,
      expiryDate: restJob.expiryDate ? dayjs(restJob.expiryDate) : undefined,
    };
  }
